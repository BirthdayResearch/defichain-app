...
const getUiConfigDetails = async () => {
  try {
    // check for UI config file
    if (checkFileExists(UI_CONFIG_FILE_NAME)) {
      const configData = getUiConfig(UI_CONFIG_FILE_NAME)
      return responseMessage(true, configData);
    }

    // check for default defi config paths
    if (checkFileExists(CONFIG_FILE_NAME)) {
      const defaultConfigData = getDefaultConfig();
      const configData = saveUiConfigData(UI_CONFIG_FILE_NAME, defaultConfigData);
      return responseMessage(true, configData);
    }
    ...
    const configData = saveUiConfigData(UI_CONFIG_FILE_NAME, defaultConfig);
    return responseMessage(true, configData);
  } catch (err) {
    console.log(err)
...
const saveUiConfigData = (path, configData) => {
  const { rpcauth, rpcbind, rpcport, testnet, regnet } = configData;
  const remotes = [];
  const uiConfigData = {
    rpcauth,
...
module.exports = { getUiConfigDetails }
